...
enum StorageOptions { SD_CARD, LOCAL_STORAGE, ENABLE_ALL }
...
class Logger {
  ...
  static Future<List<LogEntity>?> getLogsByType(LogLevel level) async {
    ...
    var filterLogs = logs!
        .where((log) => log.level.index >= level.index)
        .toList();
    return filterLogs;
  }
...
class LogsStorage {
  ...
  final String _startSign = "Reader ConsoleLog ";
  ...
  Future<List<LogEntity>?> readLogsFromFile(
    ...
    try {
      ...
      logs.removeAt(0);
      List<LogEntity> results = logs
          .map((log) => LogEntity.fromString(log))
          .toList();
      return results;
    } catch (e) {
      return null;
...
class LogEntity {
  ...
  static const String _messageIndicator = " ::: ";
  static const String _infoIndicator = " - ";
  ...
  factory LogEntity.fromString(String log) {
    List<String> logInfos = log
        .split(_messageIndicator)[0]
        .split(_infoIndicator);
